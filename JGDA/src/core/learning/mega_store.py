...
class MegaKnowledgeStore:
    """
    Motor de Conhecimento de Alta Densidade Probabilístico.
    ...
    def add_evidence(self, src_col: str, tgt_col: str, reason: str = "mapping_candidate_detected"):
        ...
        # 3. Registrar no Núcleo de Inferência Cognitivo
    ...
    def get_best_match(self, src_col: str, possible_targets: List[str]) -> Optional[str]:
        ...
        for tgt, entry in candidates.items():
            if isinstance(entry, int):
                entry = {"score": entry, "state": self._classify_state(entry)}
                
            if entry.get("state", "OBSERVED") == "REJECTED": continue
            if tgt in valid_targets:
                if entry.get("score", 0) > max_score:
                    max_score = entry.get("score", 0)
                    best_tgt = tgt

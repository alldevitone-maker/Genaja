...
class Step3View(ft.Column, RoutedViewMixin):
    """
    PASSO 3: Mapeamento de Colunas + Regras.
    Configuração de Regras de Linha e Estruturais.
    """
    def __init__(self, state, on_next, on_back):
        ...
        self.mapper = HistoricalSuggestionEngine(BRAINS_DIR)
    ...
    async def on_route_mounted(self):
        """Asynchronous execution for UI fluidity."""
        self.list_src.controls = [ft.ProgressRing(width=20, height=20, stroke_width=2)]
        self.list_tgt.controls = [ft.ProgressRing(width=20, height=20, stroke_width=2)]
        self.update()
        
        await asyncio.to_thread(self._prepare_data_lists)
        
        if self.page:
            self.page.run_task(self._render_lists)

    def _prepare_data_lists(self):
        self.cols_src = sorted(list(self.state.df_src.columns)) if self.state.df_src is not None else []
        self.cols_in_tgt = []  # Track what's been transferred

    async def _render_lists(self):

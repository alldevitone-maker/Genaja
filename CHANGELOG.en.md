...
- **ConnectorFactory**: dynamic adapter registration via Registry Pattern.
- **Wizard Step 1**: source selector (local file vs SQL database) with schema and table discovery.
- **Safe preview**: data load limited to 100 rows during selection step.
- **Credential security**: separation between `source_config_safe` and `source_config_runtime`.
